...
test("reads routing and compressed segments without losing payload controls", () => {
  ...
  assert.equal(result.destination.city, "TEST CITY");
...
test("requires the fixed 96 literal in the Format 01 header", () => {
  const invalidRouting = ["01", "9554250", "276", "068"].join(GS);
...
test("recovers the headerless, mispacked Mode 3 message from label SAMPLE-B", () => {
  ...
  assert.equal(result.secondary.shipToCity, "TEST CITY");
