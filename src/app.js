...
function updateUIFromAnalysis(analysis) {
  ...
  if (aggressiveRecovery) {
    ...
    els.recoveryReport.textContent = [
      ...
      `Located by soft-decision search: ${recovery.softDecisionPositions?.length || 0}`,
      `Soft-decision geometries tested: ${recovery.softDecisionGeometriesTested || 0}`,
    ...
    els.recoveryReport.textContent = failedRecovery
      ? [
          ...
          `Soft-decision geometries tested: ${analysis.recoveryAttempt.softDecisionAttempts || 0}`,
